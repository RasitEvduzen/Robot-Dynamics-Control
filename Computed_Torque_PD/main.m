...
% Computed - Torque Control With PD design
...
% Trajectory - 1
% p   = [0.1 1.9];      % Robot set position and Path generation      
...
for j=1:length(P1)
    ...
    tmp1 = P1dd(1,j) + Kv*ed1(j) + Kp*e1(j);
    tmp2 = P2dd(1,j) + Kv*ed2(j) + Kp*e2(j);
    ...
    xlabel('X - Axis'),ylabel('Y - Axis'),title('2 Dof Robot Arm Computed Torque Control with PD')
    drawnow
end
...
annotation('textbox', [0.015, 0.68, 0.1, 0.1], 'String', "Simulation Time = " + tsimulation)
...
subplot(424)
plot(t,(P2-q2hat),'k','linewidth',2),grid minor,hold on,title('Joint-1 Path Position Error')
